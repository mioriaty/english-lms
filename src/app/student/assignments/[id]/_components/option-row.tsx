...
export function OptionRow({ label, text, selected, disabled, onClick, reviewMode = "normal" }: OptionRowProps) {
  ...
  const rowClass = cn(
    ...
    !isReview && !selected && "border-zinc-200 bg-white text-zinc-800 hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:bg-zinc-800",
    reviewMode === "correct" && "border-[#2F5B94] bg-[#EDF2F9] text-[#2F5B94]",
    reviewMode === "missed" && "border-dashed border-[#2F5B94] bg-white text-[#2F5B94] opacity-70",
    reviewMode === "wrong" && "border-zinc-300 bg-zinc-100 text-zinc-500 line-through",
  ...
  const badgeClass = cn(
    ...
    !isReview && !selected && "border-zinc-200 text-zinc-500 dark:border-zinc-700 dark:text-zinc-400",
  ...
  return (
    <button type="button" disabled={disabled} onClick={onClick} className={rowClass}>
      ...
      {((!isReview && selected) || reviewMode === "correct") && (
        <CheckCircle2 className="mr-3 h-5 w-5 shrink-0 text-[#2F5B94]" aria-hidden="true" />
      )}
      {reviewMode === "missed" && (
        <CheckCircle2 className="mr-3 h-5 w-5 shrink-0 text-[#2F5B94] opacity-60" aria-hidden="true" />
      )}
      {reviewMode === "wrong" && (
        <XCircleIcon className="mr-3 h-5 w-5 shrink-0 text-zinc-400" aria-hidden="true" />
      )}
    </button>
  );
}
