              ...
              {result.details
                ...
                .map((d) => (
                  <li key={d.questionId} className="rounded-lg border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
                    <p className="mb-1 text-sm font-medium text-zinc-800 dark:text-zinc-200">{d.questionText}</p>
                    {d.blankResults ? (
                      <div className="mt-1 space-y-1">
                        {d.blankResults.map((r, i) => (
                          <p key={i}>
                            <span className="text-zinc-500">Blank {i + 1}: </span>
                            <span className={cn("font-medium", r.isCorrect && "text-[#2F5B94]")}>
                              &quot;{r.studentAnswer || "—"}&quot;
                            </span>
                            {!r.isCorrect && (
                              <>
