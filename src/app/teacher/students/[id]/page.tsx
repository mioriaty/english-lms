    ...
    <div className="space-y-6">
      ...
      <ul className="space-y-4">
        {student.submissions.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-zinc-500">No submissions yet.</CardContent>
          </Card>
        ) : (
                ...
                <Card>
                  <CardHeader className="pb-2">
                    ...
                    <p className="text-sm text-zinc-500">
                      Score: <span className="font-medium text-zinc-900 dark:text-zinc-100">{sub.score.toFixed(1)}</span>{" "}
                      · {new Date(sub.submittedAt).toLocaleString("en-US")}
                    </p>
                  </CardHeader>
                  <CardContent className="space-y-2 text-sm">
                    <p className="font-medium text-zinc-700 dark:text-zinc-300">Details</p>
                    <ul className="list-inside list-disc space-y-1 text-zinc-600 dark:text-zinc-400">
                      {details.map((d) => (
                        <li key={d.questionId}>
                          {d.questionId}: {d.isCorrect ? "Correct" : "Wrong"} — answered: &quot;{d.studentAnswer || "—"}&quot;
                          {!d.isCorrect && d.explain ? (
                            <span className="block pl-4 text-zinc-500">Hint: {d.explain}</span>
                          ) : null}
                        </li>
                      ))}
                    </ul>
                    {wrong.length === 0 ? (
                      <p className="text-emerald-600 dark:text-emerald-400">All correct!</p>
                    ) : null}
                  </CardContent>
                </Card>
              </li>
